...
; Parameter 1 is a pointer to the int array  register rdi
; Parameter 2 is the left index in the array  rsi
; Parameter 3 is the right index in the array rdx
; Return type is void 
mergeSort:
	push rsi ; save left index on stack
	push rdx ; save right index on stack
	cmp rsi, rdx ; if the left index is equal to the right index
	je merge_done	; base case
	add rdx, rsi ; sum together indices and put into rdx
	mov r8, 2
	idiv r8 ; divide rdx by 2 to get midpoint and store into rax
	mov r9, rax ; store midpoint into new register
	push r9 ; push midpoint value onto the stack
	mov rdx, r9 ; put midpoint value in as right index
	call mergeSort ; recurse on left half of array until array is 1
	pop r9 ; pop original midpoint value
	add r9, 1 ; add one to the midpoint to work on right side
	pop rdx ; pop original right index
	mov rsi, r9	; put midpoint value into left index
	pop rsi ;
	push rsi ;
	push rdx ;
	push r9 ;
	call mergeSort ; works on right side of array
	pop r9 ;
	pop rdx ;
	pop rsi ; get original left point
	mov rcx, rdx ; move right index into new param
	push rdx ;
	add rdx, rsi ; sum together indices and put into rdx
	idiv r8 ; divide rdx by 2 to get midpoint and store into rax
	mov rdx, rax ; store midpoint into new register
	call merge
	pop rdx ; 
